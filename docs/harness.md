...
- Long evidence is routed to `market`, `research`, and `review` so the first screen remains a decision surface rather than a wall of text.
...
- Portfolio import must preserve missing shares/cost/price/P&L/weight as null, never infer beta class from a ticker, and fail risk-budget reconciliation closed until weights and explicit `high_beta`/`normal` classifications are complete. A successful import reruns `market-levels`, `risk-watch`, `market-pulse`, `style-rotation`, then `after-close` sequentially and opens the newest report; it never places an order.
...
- The `market` route retains detailed regime, level, breadth, freshness, source, and audit evidence without crowding the `today` route. Bear-bull, fear-greed, crowding, and fixed-anchor cumulative width must show units/as-of dates and remain labelled diagnostic/unbacktested until calibrated; a low score is not a bottom signal.
...
- `--approve` is required for state changes. Approved writes use temporary files, timestamped backups, atomic replacement, and rollback on a failed refresh.
