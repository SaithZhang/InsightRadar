...
- The after-close HTML preserves the four stable first-level tasks with `today`, `portfolio`, `lookup`, and `review` routes; market and evidence context stay inside those tasks instead of becoming a fifth route.
...
- JSON exposes separate structural-action and base decision-ready coverage. Base readiness requires a dated current holding snapshot, complete account/position fields, reconciled portfolio risk, evaluated action structure, and all conditional branches; it does not require user confirmation of a personalized management proposal. `portfolio_management_plans` carries the independent `context_status` and `data_status` axes. Missing/stale context produces a deterministic proposal, while invalid/quarantined price data blocks only dependent technical capabilities and cannot be cleared by user confirmation. Original entry thesis and entry-time invalidation remain a separate Review limitation.
- `decision_workspace.repair_issues` is the actionable contract for blocked data states. Each issue names the entity, field, explicit status/reason code, provider and source/fetch time, current known value, criticality, repair permission/method, allowed input format, next action, and affected plan ids. Provider and source faults are retried through a version-bound loopback request; account snapshot gaps route to the approved portfolio-import flow. Daily-series price-basis/mapping repairs are separately recorded in ignored local state and may select Tencent forward-adjusted data only as a whole-series fallback: rows are never stitched across providers, the primary fault remains in provenance, and any invalid/stale fallback keeps the issue blocked. Retry success requires a new after-close plan version, while retry failure preserves the prior report and explicit blocked state. User confirmation, zero fill, defaults, guesses, and manual provider overrides cannot clear an issue.
...
- Inspect the JSON payload for `schema_version`, `summary_cards`, `components`, `sections`, `actions`, `unified_decision`, `unified_decision.market_regime.score_ledger`, `unified_decision.market_level_impact`, `unified_decision.style_rotation`, `unified_decision.holding_execution_plans`, `unified_decision.market_structure`, `reliability`, `data_gaps`, `holding_data_issues`, `decision_workspace.portfolio_management_plans`, and `decision_workspace.repair_issues`.
