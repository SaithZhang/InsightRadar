...
`intraday_radar` is now the Core decision seam. `AmazingData` and the per-symbol public fallback feed immutable JSONL minute/quote archives; `portfolio_memory` supplies approved holdings; `IntradaySnapshotBuilder` owns point-time derived state; four deterministic rule modules consume only the typed contract; `intraday-replay` produces IR-001 JSON/Markdown/HTML and `intraday-poll` writes the loopback runtime. Every state preserves provider `source_time` and local `fetched_at`, and no rule module calls a provider directly.
