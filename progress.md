...
## 2026-08-01 - IR-001 intraday Core acceptance passed

- Reprioritized the Core under explicit owner direction while preserving the four route ids, modular monolith, 127.0.0.1 service, self-contained HTML, private-runtime boundary, and human trade authority. ADR-0012 records the change; `feat-058` is parked rather than deleted.
- Added `IntradaySnapshot`, `ThemeSnapshot`, and `IntradayAlert` contracts with null-preserving fields, `source_time`, and `fetched_at`; local atomic JSONL minute/quote archives; a validated 20-theme universe; serial AmazingData primary access; and per-symbol Eastmoney fallback.
- Added deterministic `account_risk`, `catalyst_failure`, `opportunity_radar`, and `reentry_guard` modules. Rules consume only archived typed state and never call providers or place orders.
- The private IR-001 refresh archived 36,060 minute bars and 25 auction quotes. The ignored local triplet contains 255 no-lookahead snapshots and 15 deduplicated transitions: account protection near 09:25, catalyst yellow/orange/red escalation, software and robot confirmation, and price-only re-entry blocking. Exact account-linked evidence stays outside Git.
- Nine strategies were compared. The private results show stronger profit protection after bounded opening reductions than full hold and failed unconditional re-entry; exact monetary and account-return values remain private. Actual-operation improvement remains `unknown` because verified broker executions were not provided.
- Added bounded `intraday-poll`, local runtime overlay, `/api/intraday`, and four-page labels/panels: 今日雷达、持仓风险、机会发现、复盘验证. After-close planning, data health, evidence chains, and version history remain secondary inside the same routes.
- Verification passed: focused 7/7, full 290/290, compileall, point-time report assertions, JSON validation, fresh architecture generation, and project-memory validation.
- Added `盘中雷达.cmd` as the Windows one-shot entry: it refreshes one bounded runtime snapshot, then starts or reuses the loopback four-page app. PowerShell parsing and CLI help verification pass; the launcher does not enable continuous polling or notifications.
- `IR-001` is pass. `IR-002` is the sole next experiment for multi-session live shadow calibration, verified external mapping, and notification admission; an offline pass does not establish a general trading edge.

## 2026-08-01 - IR-002 P0 live-truth and authority hardening

- Wrote red-first regressions for all five requested failure classes. The initial focused run failed on archive overwrite/recovery, future account-peak backflow, cross-trade-date ready leakage, shadow action copy, price-only re-entry, and missing checkpoint/state-resolution behavior.
- Replaced provider/day overwrite files with append-only, content-addressed observation batches. Every row retains `trade_date`, `source_time`, `fetched_at`, `provider`, and a stable `observation_id`; supplier corrections create new observations, while the first bytes and filesystem timestamp remain recoverable.
- Removed future account-peak injection from live replay. Timed peak observations advance only when their `source_time` is visible, repeated polling cannot rewrite an archived earlier decision, and the 10:00 higher-peak regression preserves the 09:25 snapshot and alert hashes.
- Added runtime v2 with independent `trade_date`, `data_status`, `freshness_status`, and `decision_authority`. Only same-day fresh state may overlay the current workspace; stale or cross-day state becomes historical/expired. IR-002 is fixed at `shadow_only`, and the final runtime/UI/API scrub action text and position ranges rather than equating available data with advice authority.
- Added the private append-only, user-confirmed execution ledger and connected it to re-entry state. Stable execution ids exclude local confirmation time, impossible sell quantities are rejected, and `sold_at` plus `sale_price` begin the guard immediately. Re-entry requires 5-10 minute stabilization, a higher low, VWAP or rebound-high recovery, and breadth recovery. A failed first re-entry followed by a new low locks the second attempt; unlocking it requires a separate append-only confirmation that references the real first buy and a later new-low observation, so no fake second fill is created. Missing sold fraction remains `unknown`, never zero-filled.
- The Windows intraday launcher now starts a hidden single-flight checkpoint scheduler for 09:25, 09:35, and 10:00. Runtime and the existing four-route UI show last source time, last fetch time, next check, current authority, missed checkpoints, and explicit `resolved`/`invalidation` events. No fifth route, automatic order placement, publication, or push was added.
- Verification passed: focused 29/29 regressions, full 306/306 tests, compileall, focused Ruff undefined/unused-import checks, PowerShell launcher parse, JSON validation, architecture regeneration, HTML runtime contract smoke, `git diff --check`, and project-memory validation. A real Saturday `intraday-poll --iterations 1 --no-fallback` smoke correctly returned missing/blocked with `shadow_only`; it is not open-market calibration evidence.
- Remaining IR-002 work is deliberately still `in_progress`: collect multiple real 09:25/09:35/10:00 sessions, replace scenario external mapping with a verified point-in-time source, measure false escalation and missed protection, validate ledger use with a real user-confirmed fill, and admit notifications only after those gates pass.

## 2026-08-01 - IR-002 weekend usability and provider reliability repair

- Reproduced the unusable path from `ff7a58e`: the launcher waited for provider refresh before serving, Saturday used the calendar date rather than a real exchange session, domestic public clients inherited a broken proxy route, repeated Eastmoney disconnects fanned out across the universe, and old after-close dates could obscure runtime authority.
- The loopback service now serves first and owns one background worker. A scheduler lock is acquired before the initial provider call; dead-owner locks recover on restart, shutdown wakes the loop, and `scheduled/running/succeeded/partial/failed/missed` remain distinct. A terminable child enforces a 57-second hard refresh boundary.
- Provider routing is explicit and secret-safe: Galaxy/Eastmoney/Tencent/CNInfo/Iwencai use application-level domestic direct mode, Yahoo keeps the foreign system proxy, localhost/Futu stay local-only, and the UI states that OS TUN/VPN bypass is not guaranteed. A real Eastmoney direct test made three requests, received three `RemoteDisconnected` failures, opened the circuit, and skipped 87 remaining symbols.
- Runtime dates now come from the AmazingData calendar, the local immutable archive, or a bounded real K-line probe. Saturday selected 2026-07-31 as `historical_review`, loaded 90/90 symbols, and kept analysis/decision/trade authority at `historical_shadow` / `historical_shadow_only` / `none`. The 588200.SH archive contained 242 positive real minute bars through 15:00.
- Execution writes now share a cross-process ledger lock; every buy references one real sell, cumulative concurrent buys cannot exceed the sold quantity, multiple reductions keep distinct guard ids, and second-attempt overrides must reference an append-only real market failure observation.
- Real double-click measurement returned the page in 3.578 seconds; the completed background refresh took 5.4 seconds. Four-page browser QA passed with no console warnings/errors, zero horizontal overflow, consistent API/runtime/page state, visible provider routes, and no fifth route or automatic trade authority.
- Verification before review: 30/30 focused reliability tests, 336/336 full tests, compileall, focused Ruff F, hard-timeout termination, and `git diff --check`. Two independent read-only reviews found no P0 and identified scheduler/lock/checkpoint/lineage/probe/redaction/concurrency/evidence P1s; all were repaired and sent for re-review before commit.
- Final re-review also exercised retry exhaustion across application restart and proxy diagnostics in bare-authority, nested-label, Python-repr, and JSON forms. The exhausted checkpoint no longer records a third failed attempt or remains the displayed next check, every tested proxy address/credential form is redacted, and both reviewers reported no remaining P0/P1. The final full suite remained 336/336.
- IR-002 remains `in_progress`: open-market calibration, verified external mapping, measured timing/false escalation/missed protection, a real user-confirmed ledger trial, sleep/process-death checkpoint observation, and notification admission are still required.

## 2026-08-01 - feat-058 bounded Today Workbench replacement

- Replaced only the `today` route with the owner-supplied after-close/weekend three-column workbench. First-level entries are 今日工作台、组合风险、标的研究、复盘账本; no fifth risk-command route was added and the three detail routes remain intact.
- Added a typed `today_workbench` contract and deterministic account P&L, peak, giveback, attribution, priority, data-quality, rule, and decision-requirement calculations. Position and opportunity attention share one structure; absent verified opportunities render an explicit empty state.
- Added version-scoped disabled responses and a fail-closed rule state machine. Blocked rules cannot be accepted, unconfirmed/modified/disabled rules cannot enter monitor eligibility, and a modification request cannot reconfirm the stale version. AI is unused and trade authority remains `none`.
- Browser QA compared the supplied 1440 x 900 screenshot and real implementation together. It found and fixed one hash-anchor scroll defect, then passed at 1440 x 900, 1024 x 768, and 390 x 844 with zero horizontal overflow, four-route navigation, parameterized holding links, native evidence disclosure, and zero page console warnings/errors. Evidence is recorded in `design-qa.md`.
- Verification: fresh after-close triplet, focused 33/33 tests, full 342/342 tests, compileall, new-module Mypy, full Ruff on the new module/tests, isolated package build, architecture regeneration, and project-memory validation pass. The repository-wide Ruff audit still reports 422 existing debt findings; the broader touched-module Mypy audit reports 25 renderer/workspace errors outside the clean new typed module.
- IR-002 remains the sole active experiment. Premarket/intraday Today modes, real AI integration, automatic notifications, and trade execution were not implemented.

## 2026-08-01 - Local refresh worker startup recovery

- Reproduced the owner-reported full-refresh failure through the live `127.0.0.1:8765` API: `market-levels` exited immediately with Windows status `0xC0000142`, no stdout/stderr, and all later workflows stayed pending. The approved portfolio was already valid and version-bound, so the failure was not an import rollback or market-level validation error.
- Minimized the failure with controlled services. The same workflow succeeded standalone, from a background thread, from a fresh plain service, from a fresh `--intraday` service, and from a fresh service that opened the browser. Only the long-running original service process failed, establishing a process-local Windows initialization fault rather than a deterministic data or intraday-mode defect.
- `RefreshCoordinator` now retries this exact no-output process-initialization failure once. Persistent failure records the actual `0xC0000142` cause and tells the owner to restart InsightRadar instead of reporting the misleading generic `market-levels failed` message.
- The portfolio import page now offers an explicit full-refresh retry for ordinary failures/interruption and suppresses futile retries when a local-service restart is required. Holdings remain saved and the last-good report remains visible throughout.
- Gracefully restarted the local service with the repaired code and ran a real full refresh bound to the imported portfolio version. All six steps completed with return code zero and produced a fresh `after-close` triplet. Any remaining private beta-classification gap still blocks risk reconciliation by design.
- Verification passed: two red-first process-failure regressions, importer recovery UI contract, 344/344 full tests, compileall, focused Ruff F/I, focused Mypy, `git diff --check`, and project-memory validation. Diagnostic services and temporary logs were removed; no automatic trading or authority bypass was introduced.

## 2026-08-01 - Deterministic portfolio beta and risk reconciliation

- Replaced the import UI's manual beta selectors with an automatic, evidence-backed workflow. `portfolio-beta` uses `000300.SH`, simple daily returns, a 120-session window, a 60-observation minimum, and a pre-registered `beta >= 1.20` high-beta threshold; no ticker/name/industry guess, AI call, notification, order, or trade authority is involved.
- Added typed beta results and persisted beta, R², benchmark, window, observations, benchmark/asset as-of, source, data-quality status, fit quality, formula, and classification atomically with the private portfolio and risk profile. Stale dates, insufficient history, invalid variance, non-finite results, unsupported codes, and source failure remain `unknown` and continue to block reconciliation.
- Added `portfolio-beta` before `risk-watch` in the seven-step full refresh and before stale risk refreshes. Since this step changes canonical evidence, `RefreshCoordinator` now rebinds final `after-close` artifact validation to the post-beta `portfolio_version`.
- The real local run used Tencent public daily K-lines and produced ready 120-observation evidence for 4/4 private holdings through 2026-07-31. Beta risk reconciliation is now `reconciled`; strict readiness remains 0/4 because holding context and other independent evidence blockers are not bypassed.
- The repaired loopback service completed all seven real workflows with return code zero. The fresh after-close JSON/Markdown/HTML triplet matches the post-beta portfolio version; the four-route page renders all four beta/R² rows, no desktop horizontal overflow, and contained table scrolling at 768px.
- Verification: 352/352 tests, compileall, focused Ruff F/I, focused Mypy for the new calculation/persistence/refresh seam, JSON and diff checks, architecture regeneration, responsive browser QA, and project-memory validation pass. Broader legacy renderer/workspace Mypy findings remain outside this change.

## 2026-08-01 - Current risk context separated from historical entry context

- Reproduced the real blocker and confirmed that the position record, current risk rule, and review state were present; only the original entry-time invalidation remained explicitly unknown. The old reliability predicate incorrectly made that historical gap a current-plan blocker.
- Split the contract into `current_context_complete` and `historical_context_complete`, with explicit missing-field lists. `context_complete` remains a compatibility alias for current context. Current risk rule/review gaps still block; unknown original thesis/invalidation moves to optional Review evidence and is never backfilled.
- Added Review disclosure for historical entry-context coverage and field-level gaps. The product now records that incomplete history limits strategy/execution-quality conclusions without blocking a current risk plan.
...
## 2026-08-02 - feat-058 holding-management closure

- Replaced the old all-or-nothing current-context blocker with independent management-consent and data-quality axes. Missing or conflicting context now produces a deterministic system proposal; account, mapping, adjustment-basis, and quarantined-price faults remain fail closed only for the capabilities that depend on them.
- Added the loopback Portfolio flow for adopt, fixed single-choice adjustment, or uncertain monitoring. The server validates enums and applicability, atomically persists the compatible private context, preserves input on failure, and starts an after-close-only regeneration. User confirmation cannot clear a data quarantine.
- Portfolio now separates “持仓管理方案待确认” from “行情数据异常”. The per-holding drawer shows basis, trigger, duration, action, invalidation, next review, source time, generation time, and confidence; optional text is not required and internal contract names are not shown to ordinary users.
- A private local after-close acceptance confirmed the expected consent/data-quality split without adding symbols, account values, or raw reports to Git. Browser QA exercised the real loopback Portfolio page and adjustment drawer without saving, confirmed the data-blocked disclosure and disabled inapplicable profit protection, and found no console warnings or errors.
- Verification passes: 374/374 full tests, compileall, focused Ruff F/I, scoped Mypy for the clean persistence/refresh seams, JSON parsing, `git diff --check`, and project-memory validation. Broader legacy renderer/workspace Mypy debt remains outside this bounded change. `feat-058` stays `in_progress` pending owner visual acceptance; no AI dependency, notification, order, fifth route, commit, push, or publication was added.

## 2026-08-07 - feat-059 read-only intraday evidence verified locally

- Added a provider-neutral `IntradayTape`/`EvidenceEnvelope` layer under the existing intraday module. Eastmoney `trends2` is primary; Tencent minute/day is a whole-series fallback. Raw provider fields stop at adapters, incomplete amounts fail closed, timestamps/counters are validated, and conflicts retain both provenance records without row merging.
- Added deterministic tape, comparison, same-time Shanghai+Shenzhen amount, and user-confirmed trade-review services. Compare aligns a common minute; review uses only the prior completed minute for decision context and separates later 5/15/30-minute outcome, buy MAE/MFE, and sell-after extrema. Missing extrema, benchmark outages, stale feeds, lunch/future times, offline data, and bounded input limits remain explicit.
- Added a JSON CLI and exactly four typed, read-only MCP tools over stdio or loopback Streamable HTTP. Concrete Pydantic input/output schemas, finite-number checks, `read_only_hint`, `trade_authority=none`, and subprocess transport tests preserve the non-trading boundary.
- The ignored private owner-supplied acceptance passed with no pending horizon; exact executions and detailed output remain local. A real single-tape request completed in 0.68 seconds; real compare and market-amount requests completed in 1.34 and 1.25 seconds. Market amount correctly returned degraded because Eastmoney lacked two common sessions and Tencent supplied the fallback.
- A live reopen-edge check found Tencent labelling the partial tail with the next minute. Failing regressions now prove that the adapter drops timestamps after `fetched_at` before counter validation, differencing, and unit inference, marks the tape partial, and preserves all completed minutes; the repaired fallback returned degraded evidence instead of discarding or contaminating the whole series.
- Verification passes: 427/427 full tests with one opt-in live skip, separate live-provider smoke, compileall, focused Ruff, focused Mypy, JSON validation, generated architecture, project-memory validation, CLI entrypoint, stdio MCP, loopback HTTP MCP, and `git diff --check`. Repository-wide Ruff still has 417 pre-existing findings outside this increment.
- `feat-059` remains `in_progress` until Draft PR/owner acceptance. ChatGPT Web still needs an allowed developer-mode/full-MCP workspace path plus Secure MCP Tunnel or an authenticated reviewed remote endpoint; localhost is not claimed reachable.

## 2026-08-08 - feat-058 blocked-to-repair vertical closure

- Reproduced the real terminal blocker: decision plans retained ProviderResult quarantine and stale-source failures, but the workspace reduced them to prose, Portfolio cards were read-only, and the loopback service had no issue-scoped retry contract. User management confirmation already worked and was intentionally left independent.
- Preserved provider lineage from the holding decision into `decision-workspace/v1`. Added stable plan-linked `repair_issues` covering market-contract faults, stale/missing decision sources, broker snapshot gaps, and risk reconciliation. Each issue keeps entity, field, status/reason code, source/fetch time, price basis/current known value, criticality, repair authority/method, allowed input format, next action, and affected plan ids; unknown is never zero-filled.
- Added the Portfolio data-quality entry, repair drawer, and version-bound `POST /api/repair-recheck`. Provider/source problems use a bounded system retry and regenerate after-close; account snapshot gaps route to the approved importer. Retry failure is visible and keeps the last-good report plus blocked state. Management adopt/modify/uncertain persistence remains unchanged and cannot clear quarantine.
- A fresh private after-close run linked every blocked plan to at least one concrete issue. Browser QA first exercised the fail-closed path. The admitted system repair then recorded an ignored issue-scoped request, selected Tencent forward-adjusted data as a whole-series fallback without stitching, preserved the AmazingData quarantine in provenance, and regenerated after-close. A bounded seven-step source refresh cleared all stale decision sources. The final private `20260808-104209` report has zero repair issues, zero blocked plans, and all current holding plans decision-ready; pending management proposals remain non-blocking personalization. Private symbols, values, generated reports, and screenshots remain outside tracked scope.
- Verification passes: 92 focused/adjacent tests and 441 full tests with one opt-in skip, compileall, focused Ruff F/I, real after-close generation, report contract assertions, desktop 1440x900 and mobile 390x844 browser QA with no horizontal overflow, empty browser warnings/errors, and `git diff --check`. The repository-wide ignored bear-bull state mutation remains pre-existing test-isolation debt.
- `feat-058` remains `in_progress` only for final owner acceptance. Stop here: no P1/P2, fifth route, notification, automatic trade, commit, push, or publication was added.

## 2026-08-08 - feat-058 black-box after-close and stale-workspace P0 closure

- Read-only forensics confirmed that the approved import had persisted a newer canonical portfolio, while the latest published after-close workspace still belonged to the previous portfolio version. The durable seven-step refresh failed first at `after-close`; the exact traceback reached AmazingData `get_tgw_type_code`, where a code without an exchange suffix raised `ValueError: not enough values to unpack (expected 2, got 1)`.
- The broker market field used `上海Ａ股`; the import/load normalizers recognized only labels beginning with `沪` (or selected stock-code prefixes), so Shanghai fund/ETF codes could remain one-part. Import preview and backward-compatible JSON loading now normalize explicit `上海`/`深圳` labels before provider calls. Exceptions are not swallowed and holdings are not skipped.
- Workspace publication now binds current authority to the exact canonical `portfolio_version`. A mismatched workspace is retained as a structured `superseded` historical snapshot, all dependent plans are blocked and old accepted authority is revoked, the page shows the failed refresh step and retry path, and plan-action controls are absent. `/api/plan-response` independently reloads the canonical portfolio and rejects stale versions before any ledger write.
- Regression coverage includes import/load provider-code normalization, successful synthetic after-close report generation, failed-refresh supersession, historical-only UI, server-side ledger rejection, and successful matching-version recovery with the old report retained.
- Verification passes: 99 focused/adjacent tests, 453 full tests with one opt-in skip, compileall, `git diff --check`, and a real loopback full refresh. The real run completed 7/7, produced a matching private report triplet, restored `workspace_validity=current`, and explicit calendar resolution returned Friday 2026-08-07 for the Saturday run. No P1/P2, route, trade authority, commit, push, or publication was added.
