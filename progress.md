...
- Verification passes: 425/425 full tests with one opt-in live skip, separate live-provider smoke, compileall, focused Ruff, focused Mypy, JSON validation, generated architecture, project-memory validation, CLI entrypoint, stdio MCP, loopback HTTP MCP, and `git diff --check`. Repository-wide Ruff still has 417 pre-existing findings outside this increment.
