...
## 2026-08-01 Handoff - IR-002 P0 hardened; live calibration remains open

- The worktree now contains the P0 implementation requested after `dcc627693cc546ebeffe6df73231284616efe4ab`; nothing was committed, pushed, or published. Preserve the frozen four route ids and keep IR-002 at `shadow_only`.
- Point-in-time raw data is now immutable append-only observation storage. Corrections are later observations, not rewrites, and reads can be bounded by both market `source_time` and local observation `fetched_at`.
- Live polling no longer injects a later account peak into the session prefix. Runtime v2 separates date, availability, freshness, and authority; the loopback workspace rejects stale/cross-day ready state and scrubs trade-action copy before display.
- User-confirmed executions are append-only local evidence, not orders. Re-entry uses real `sold_at`/`sale_price`, requires stabilization plus structural and breadth confirmation, and locks a second attempt after a failed first re-entry. The override is a separate append-only event referencing the real first buy and a later new-low observation; it cannot be represented by a fake additional execution.
- Double-click intraday startup launches one serial refresh and a hidden 09:25/09:35/10:00 scheduler. The existing four-route UI reports source/fetch/next-check/authority and activated/escalated/updated/resolved/invalidation transitions.
- Final gates are green: focused 29/29, full 306/306, compileall, focused Ruff F checks, launcher parse, JSON checks, HTML contract smoke, architecture regeneration, `git diff --check`, and project-memory validation. The Saturday no-fallback CLI smoke correctly produced missing/blocked rather than fake ready data.
- Do not promote IR-002 to pass yet. Still required: multiple open-market shadow sessions, verified external point-in-time mapping, measured timing/false escalation/missed protection, a real confirmed execution-ledger trial, and notification admission. Process death or machine sleep can still cause missed checkpoints; the runtime exposes them, but no OS-level task retry has been claimed.

## 2026-08-01 Handoff - weekend radar usable; IR-002 calibration still open

- Start from the new post-`ff7a58e` commit once recorded. The real `盘中雷达.cmd` path serves `127.0.0.1:8765` in 3.578 seconds and then refreshes in a bounded child; the page process owns the pre-registered checkpoint scheduler and stops only its own child/locks.
- Provider routes are declared per source and visible without proxy secrets. “国内直连” means application clients bypass environment proxies; it does not claim OS TUN/VPN bypass. Yahoo remains system-proxy and localhost/Futu local-only.
- Saturday is not fake live. The runtime/API/workspace selected real 2026-07-31 history, loaded 90/90 symbols, and exposed `historical_shadow / historical_shadow_only / none`. The archived 588200.SH series had 242 positive bars through 15:00. Eastmoney made three real direct attempts, opened the circuit on repeated disconnects, and skipped the remaining 87.
- Execution/re-entry state is append-only and cross-process single-flight. Buy rows must reference a matching sell, concurrent cumulative buy quantity cannot exceed that sell, multiple sells retain separate guard lineage, and an override references a real market-observed failure rather than a user-entered synthetic event.
- Acceptance gates include 30 focused reliability tests, 336 full tests, compileall, Ruff F, hard-timeout termination, generated architecture, project-memory validation, JSON checks, diff check, real four-page browser QA, and two read-only re-reviews with no remaining P0/P1. If any of these have not yet run in the current worktree, do not describe the feature as committed-complete.
- Keep IR-002 `in_progress`. Still missing: multiple open-session checkpoints, verified external mapping, measured timing/false escalation/missed protection, a real user-confirmed ledger trial, sleep/process-death observations, and notification admission. No automatic fills, fifth route, publication, or push is authorized.

## 2026-08-01 Handoff - Today Workbench implemented and browser verified

- The current uncommitted work replaces only `today` with the postclose-2026-07-31 three-column workbench. Preserve the user's untracked `docs/product-prototypes/` source, the four route ids, and the unchanged Portfolio/Lookup/Review responsibility boundary.
- `stock_assist/today_workbench.py` is the typed deterministic seam. Keep facts/calculations, rule/response state, and display templates separated. Missing opportunity evidence must stay explicit; AI remains unused and trade authority stays `none`.
- Response status now includes `disabled`. Blocked acceptance fails closed, modified old versions cannot be reconfirmed, and only confirmed + ready + unblocked rules become monitor eligible.
- A fresh report triplet was generated. Focused 33/33 and full 342/342 tests, compileall, new-module Mypy, full Ruff on the new module/tests, isolated package build, architecture regeneration, and project-memory validation pass. The repository-wide Ruff audit retains 422 existing debt findings; the broader touched-module Mypy audit retains 25 renderer/workspace errors outside the clean new module.
- `design-qa.md` records the original/final 1440 x 900 comparison, the repaired hash-anchor scroll issue, responsive 1024 x 768 and 390 x 844 checks, four-route/parameterized-link behavior, and empty page console diagnostics.
- Public commit scope excludes the user's untracked prototype source and all ignored private reports/screenshots. No PR, automatic notification, AI API, intraday Today implementation, or trading action was added. IR-002 remains the sole active experiment.

## 2026-08-01 Handoff - refresh worker startup failure repaired

- The imported portfolio was saved correctly; the observed `market-levels` failure was a process-local Windows child initialization exit `0xC0000142` with no workflow diagnostics. Standalone, background-thread, fresh plain, fresh intraday, and fresh browser-opening service controls all passed, so do not attribute this incident to portfolio data or `market-levels` rules.
- `RefreshCoordinator` performs one bounded retry for this exact empty-output exit. If it persists, the durable job now says to restart InsightRadar and preserves the approved portfolio plus last-good report. The import UI exposes retry for ordinary failed/interrupted jobs but avoids looping a restart-required process.
- The old local service and all diagnostic services were shut down cleanly. A repaired `--intraday` service is running on `127.0.0.1:8765`; a real full refresh completed all six workflows with return code zero and generated a fresh portfolio-version-matched report triplet.
- Final gates: 344/344 tests, compileall, focused Ruff F/I, focused Mypy, diff check, and project-memory validation. Temporary diagnostic logs were deleted. A private beta-classification gap may still keep risk reconciliation blocked; do not infer its value or bypass that rule.

## 2026-08-01 Handoff - automatic beta evidence implemented

- Manual beta selection is retired from the normal import page. Approved imports save first with `unknown`, then the background `portfolio-beta` step computes deterministic evidence before `risk-watch` using `000300.SH`, 120 simple daily returns, at least 60 observations, and a 1.20 high-beta threshold.
- Canonical private holdings now retain beta/R²/benchmark/window/observations/as-of/source/data-quality/fit-quality/formula evidence. Non-ready evidence always writes `unknown`; deprecated CLI classification input is ignored and cannot bypass the rule.
- The current private snapshot has ready 120-observation evidence for 4/4 holdings through 2026-07-31, and beta risk reconciliation is `reconciled`. Do not describe all plans as ready: missing holding context and other independent blockers still keep strict readiness at 0/4.
- Full refresh order is now `portfolio-beta -> market-levels -> risk-watch -> market-pulse -> style-rotation -> ai-capex-watch -> after-close`. The coordinator rebinds the expected portfolio version after beta persistence; the latest real 7/7 run produced a matching after-close triplet.
- Final gates: 352/352 tests, compileall, focused Ruff F/I, focused Mypy on the new seam, JSON/diff checks, generated architecture, responsive desktop/768px browser QA, and project-memory validation. The local `--intraday` service is running at `127.0.0.1:8765` with the new UI. No commit or push was performed, and the user's untracked `docs/product-prototypes/` remains untouched.

## 2026-08-01 Handoff - current risk context no longer depends on unknown entry history

- ADR-0014 separates present decision context from historical entry context. Current risk rule plus a usable review state control strict readiness; original entry thesis/invalidation remain optional, explicit Review evidence and cannot be reconstructed by AI or hindsight.
- The JSON contract adds current/historical completeness and missing-field lists while retaining `context_complete` as the current-context compatibility alias. Review renders historical coverage and explains that the gap limits strategy/execution evaluation only.
...
- Live reopen-edge regressions drop Tencent's next-minute partial tail before counter and unit inference while preserving completed minutes and explicit degraded provenance; do not shift the provider timestamp backward or reclassify it as completed data.
- Verified gates: 427 full tests (one opt-in live skip), live provider smoke, compileall, focused Ruff/Mypy, CLI subprocess, stdio and HTTP MCP subprocesses, architecture regeneration, JSON/diff checks, and project-memory validation. Repository-wide Ruff retains 417 unrelated baseline findings.
...
## 2026-08-08 Handoff - feat-058 repair closure ready for owner acceptance

- The owner explicitly selected only the vertical chain `持仓 -> 数据 -> 风险上下文 -> 盘前/盘后 -> blocked -> 修复入口 -> 修复 -> 重新生成`; no next increment is admitted. The worktree is intentionally uncommitted and unpublished.
- Runtime contract: `HoldingDecision.data_evidence` preserves the ProviderResult provider/schema/status/source/fetch/trade date/price basis/gaps/errors. `decision_workspace.repair_issues` converts every relevant fail-closed condition into a stable, plan-linked issue. Provider and source issues forbid manual override; missing broker fields use the existing preview/approval importer.
- Loopback contract: `POST /api/repair-recheck` requires an exact current workspace version and issue id. Price/provider faults run after-close only; source faults target only the named unhealthy workflow before after-close; risk reconciliation includes beta before risk-watch; stale issue ids fail closed. Refresh failure is propagated and never changes the issue to ready.
- UX contract: Portfolio keeps management consent and data quality in separate sections. Each data card opens a drawer with the problem, exact field/current known value, why it blocks, source/time/basis/evidence, manual-repair permission, allowed input format, save semantics, and the post-repair next step. The action is “重新检查并生成”; successful regeneration reloads the new plan, while unresolved evidence remains visibly blocked.
- Verification: 92 focused/adjacent tests and 441 full tests with one opt-in live skip, compileall, focused Ruff F/I, real after-close generation, structured report assertions, and `git diff --check`. Real browser QA passed at 1440x900 and 390x844 with no horizontal overflow or console warnings/errors. The UI first exercised fail-closed retention; the admitted repair then used a recorded Tencent forward-adjusted whole-series fallback without stitching, preserved the primary quarantine in provenance, refreshed seven required steps, and produced the private `20260808-104209` report with zero repair issues, zero blocked plans, and all current holding plans ready.
- Changed runtime/tests: `stock_assist/holding_decision.py`, `stock_assist/decision_workspace.py`, `stock_assist/data_sources/a_share_klines.py`, `stock_assist/workflows/after_close.py`, `stock_assist/portfolio_import_server.py`, `stock_assist/after_close_workbench_html.py`, `tests/test_portfolio_management.py`, `tests/test_daily_kline_contract.py`, and `tests/test_after_close_reliability.py`. Updated contract/state evidence: ADR-0016, `docs/harness.md`, `docs/V3.1_DELTA.md`, decision/product memory, `feature_list.json`, `CURRENT_STATE.md`, `progress.md`, and this handoff.
- Remaining P0: owner visual acceptance only. Monday runtime still fails closed if both primary and fallback evidence are invalid or required sources cannot refresh; continue to show `unknown`/`stale`/`quarantined` and never bypass them. Do not start P1/P2, commit, push, publish, or alter trade authority without a new owner request.

## 2026-08-08 Handoff - feat-058 black-box P0 repaired and real refresh verified

- The real import was already saved; the user saw old holdings because after-close crashed and `_latest_workspace()` published the previous report without comparing portfolio versions. The crash was not the CLI's generic TSV advice: AmazingData destructured a one-part security code after the broker label `上海Ａ股` failed the `沪`-only normalization rule.
- Preserve the repaired contract: import and JSON load normalize explicit `上海`/`深圳` market labels; current authority requires exact `current portfolio_version == workspace portfolio_version`; mismatches remain visible only as `superseded` history. Do not delete old reports or infer missing values.
- The historical page must keep its prominent validity banner, failed-step/error summary, and full-refresh recovery control while exposing no `data-plan-response` actions. The server guard is authoritative: it reloads the canonical portfolio and rejects any stale plan response before `append_plan_response` or the refresh ledger is touched.
- Verification is green: 99 focused/adjacent tests, 453 full tests with one opt-in skip, compileall, diff checks, and a real loopback full refresh completed all seven steps. The current private portfolio and new workspace versions/position counts match, the page is current, the artifact triplet exists, and the explicit provider calendar resolves 2026-08-07 as the latest completed session on Saturday 2026-08-08.
- `feat-058` remains `in_progress` for owner acceptance. There is no currently observed P0 blocking the Monday trial. Do not start P1/P2, commit, push, publish, or change trade authority without a new owner request.
