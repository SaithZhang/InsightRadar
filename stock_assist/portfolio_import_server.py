...
def serve_portfolio_import(*, port: int = 8765, open_browser: bool = True) -> None:
...
<label><input id="approved" type="checkbox"> 2. 我已核对新旧差异并明确批准保存</label><button id="apply" type="button" disabled>3. 保存并刷新全部报告</button></section><section><div id="output" class="output"><div class="summary">等待本地解析。</div></div></section></main>
<script>
const token={json.dumps(token)};let last=null;let betaValues={{}};const $=id=>document.getElementById(id);
const classifications=()=>betaValues;
const post=async(path,approved=false)=>{{const r=await fetch(path,{{method:'POST',headers:{{'Content-Type':'application/json','X-InsightRadar-Token':token}},body:JSON.stringify({{text:$('text').value,classifications:classifications(),approved}})}});const data=await r.json();if(!r.ok)throw new Error(data.error||'请求失败');return data}};
...
$('apply').addEventListener('click',async()=>{{if(!$('approved').checked||!last)return;$('apply').disabled=true;$('status').textContent='正在原子保存并串行刷新，请勿关闭本页…';try{{const result=await post('/api/apply',true);const output=$('output');output.replaceChildren();const done=document.createElement('div');done.className='summary';done.textContent=`导入完成，已保存 ${{result.portfolio_path}}。`;const link=document.createElement('a');link.className='button';link.href='/#today';link.textContent='返回行动简报';output.append(done,link);$('status').textContent='导入与刷新完成。'}}catch(e){{$('status').textContent='失败：'+e.message;$('apply').disabled=false}}}});
