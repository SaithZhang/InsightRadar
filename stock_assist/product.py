...
COMMANDS: tuple[ProductCommand, ...] = (
    ...
    ProductCommand(
        ...
        inputs=("local broker TSV", "data/portfolio.json", "data/risk_watch_profile.json"),
        outputs=("atomic portfolio/risk-profile save with backups", "automatic portfolio-beta evidence", "fresh market-levels/risk-watch/market-pulse/after-close reports"),
        retry="Fix validation or reconciliation gaps; the command never writes without --approve and never places orders.",
    ),
    ProductCommand(
        name="portfolio-beta",
        module_key="portfolio",
        help="calculate and persist evidence-backed portfolio beta classifications",
        run_hint="Runs automatically after an approved import and before risk-watch; manual execution is for data repair only.",
        inputs=("configs/portfolio_beta.json", "data/portfolio.json", "public adjusted A-share and CSI 300 daily K-lines"),
        outputs=("atomic beta evidence and risk-profile reconciliation", "reports/*-portfolio-beta.json", "reports/*-portfolio-beta.md", "reports/*-portfolio-beta.html"),
        retry="Inspect per-position quality gaps; stale, insufficient, or invalid data remains unknown and blocks reconciliation.",
    ),
    ProductCommand(
...
FILES: tuple[ProductFile, ...] = (
    ...
    ProductFile("configs/portfolio_beta.json", "product_config", "portfolio", "Deterministic benchmark, window, sample, threshold, and quality contract for portfolio beta."),
