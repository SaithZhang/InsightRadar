...
def _document(workspace: Mapping[str, object]) -> str:
    ...
    today_workbench = build_today_workbench(workspace)
    if _mapping(workspace.get("workspace_validity")).get("status") == "superseded":
        today_workbench = _disable_superseded_plan_actions(today_workbench)
    workspace["today_workbench"] = today_workbench
...
<section class="view active" id="route-today" data-route-panel="today">
  {_workspace_validity_banner(workspace)}
...
def _disable_superseded_plan_actions(
    today: Mapping[str, object],
) -> dict[str, object]:
    result = dict(today)
    blocker = "历史快照只供回看；当前持仓刷新成功前不能提交计划回应。"
    requirements = []
    for raw in _dict_rows(today.get("decision_requirements")):
        item = dict(raw)
        item["status"] = "disabled"
        item["allowed_responses"] = []
        item["prompt"] = blocker
        reasons = _string_rows(item.get("blocking_reasons"))
        item["blocking_reasons"] = [*reasons, *([] if blocker in reasons else [blocker])]
        requirements.append(item)
    result["decision_requirements"] = requirements
    result["data_quality"] = "blocked"
    gaps = _string_rows(result.get("data_gaps"))
    result["data_gaps"] = [*gaps, *([] if blocker in gaps else [blocker])]
    return result


def _workspace_validity_banner(workspace: Mapping[str, object]) -> str:
    validity = _mapping(workspace.get("workspace_validity"))
    if validity.get("status") != "superseded":
        return ""
    refresh = _mapping(validity.get("latest_refresh"))
    refresh_status = str(refresh.get("status") or "unknown")
    if refresh_status == "failed":
        headline = "当前持仓已更新，但决策工作台刷新失败"
    elif refresh_status in {"pending", "running"}:
        headline = "当前持仓已更新，决策工作台正在刷新"
    else:
        headline = "当前持仓已更新，当前工作台尚未重新生成"
    failed_step = str(
        refresh.get("failed_step") or refresh.get("current_step") or "unknown"
    )
    error = str(refresh.get("error") or "未记录可公开的失败详情。")
    return f"""<section class="workspace-validity-banner" role="alert">
    <div><span>历史快照 · 仅供回看</span><strong>{escape(headline)}</strong>
      <p>旧报告仍保留用于审计，但不代表当前持仓，也不具备计划确认权限。请点击顶部“全量刷新”重新生成当前工作台。</p></div>
    <dl><div><dt>当前持仓日期</dt><dd>{escape(str(validity.get('current_portfolio_as_of') or 'unknown'))}</dd></div>
      <div><dt>历史工作台日期</dt><dd>{escape(str(validity.get('workspace_effective_market_date') or 'unknown'))}</dd></div>
      <div><dt>刷新状态</dt><dd>{escape(refresh_status)}</dd></div>
      <div><dt>失败步骤</dt><dd>{escape(failed_step)}</dd></div></dl>
    <details><summary>查看刷新失败信息</summary><p>{escape(error)}</p></details>
  </section>"""
...
.workspace-validity-banner{margin-bottom:12px;padding:14px 16px;border:1px solid #8b513a;border-radius:var(--radius);background:#2a1714;color:var(--muted)}.workspace-validity-banner>div>span{display:block;margin-bottom:4px;color:#ffb38f;font-size:12px;font-weight:800}.workspace-validity-banner strong{display:block;color:#ffe2d2;font-size:17px}.workspace-validity-banner p{margin:6px 0 0}.workspace-validity-banner dl{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin:12px 0 0}.workspace-validity-banner dl div{padding:8px;border:1px solid #57362e;border-radius:6px;background:#1d1211}.workspace-validity-banner dt{font-size:11px}.workspace-validity-banner dd{margin:3px 0 0;color:var(--text);font-weight:700}.workspace-validity-banner details{margin-top:10px}.workspace-validity-banner details p{white-space:pre-wrap;word-break:break-word}.today-phase-banner{min-height:48px;margin-bottom:12px;padding:10px 14px;border:1px solid #4a5437;border-radius:var(--radius);color:var(--muted);background:#121d22;display:flex;align-items:center;gap:9px}.today-phase-banner strong{color:var(--text)}.today-phase-banner em{margin-left:auto;font-style:normal}.today-phase-banner .phase-dot{width:8px;height:8px;border-radius:50%;background:var(--amber)}.today-phase-banner.after_close{border-color:#3d586f;background:#0a1a28}.today-phase-banner.after_close .phase-dot{background:var(--blue)}
...
@media(max-width:820px){.workspace-validity-banner dl{grid-template-columns:repeat(2,minmax(0,1fr))}.today-phase-banner{align-items:flex-start;flex-wrap:wrap}.today-phase-banner em{margin-left:0}.today-column-head{min-height:0}.today-data-details>div{grid-template-columns:repeat(2,minmax(0,1fr))}}
